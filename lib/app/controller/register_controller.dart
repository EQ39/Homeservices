...
class RegisterController extends GetxController implements GetxService {
      ...
      if (smsName == '2') {
        ...
        if (response.statusCode == 200) {
          ...
          if (myMap['data'] != '' && myMap['data'] == true) {
            smsId = myMap['otp_id'];
            FocusManager.instance.primaryFocus?.unfocus();
            openOTPModal(countryCodeMobile.toString() +
                mobileTextEditor.text.toString());
          } else {
            if (myMap['data'] != '' &&
                myMap['data'] == false &&
                myMap['status'] == 500) {
  ...
  Future<void> createAccount() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (referalCodeTextEditor.text.isNotEmpty &&
          referalCodeTextEditor.text != '') {
